<?php 
   ...
   <!-- /mobile-menu-holder-->
   <header id="header-7" class="headerHolder header-7">
      <div class="nav-button-holder nav-button-holder-7">
         <?php if (has_nav_menu('primary-menu')) { ?>
      ...
      <?php 	$mt_logo_img = get_theme_mod( 'custom_logo' );
         ...
         ?>
      <div class="logo logo-7"><a href="<?php echo esc_url( home_url( '/' ) ); ?>"><img class="img-fluid" src="<?php echo esc_url($mt_logo_img_url[0]); ?>" width="<?php echo esc_attr($mt_logo_img_url[1]); ?>" height="<?php echo esc_attr($mt_logo_img_url[2]); ?>" alt="<?php bloginfo('name'); ?>" /></a></div>
      <?php else: ?>
      <div class="logo logo-7">
         <div class="logo-txt"><a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php bloginfo('name'); ?></a></div>
      </div>
      <?php endif; ?>
      <nav class="nav-holder nav-holder-7">
         <?php if (has_nav_menu('primary-menu')) {
            wp_nav_menu(array('theme_location' => 'primary-menu', 'menu_class' => 'menu-nav menu-nav-7', 'container' => 'false'));
            
            }   ?>
      </nav>

	  <div class="social-btn-top1 social-btn-top7">
	  
	  <?php $mt_header_info = get_theme_mod( 'mt_header_info');

		if (!empty($mt_header_info) ): ?>
	  
	  <div class="binfo binfo6">
	 ...
	 <?php endif; ?>
      ...
      <div class="btn-header btn-header1 btn-header7">
